...
> 📸 **Screenshot:** Capture the terminal showing `sudo ufw status` with all rules listed and the terminal showing `sudo ss -lntp` with ports 3000 and 5432 bound to 127.0.0.1.
> ![UFW status and listening ports](screenshots/13-ufw-status-and-ports.png)
